...
public class ExpenseTypeParaResourceTest extends ResourceTest {
	...
	public void testFindAllTypeParameter() {
		Response response = target("expenseType").path("2")
				.path("typeParameter").register(JacksonFeature.class).request()
				.header("user", "demo").get();
		assertEquals(Status.OK.getStatusCode(), response.getStatus());
		PageModel<TypeParameterSendto> rets = response
				.readEntity(new GenericType<PageModel<TypeParameterSendto>>() {
				});
	...
	public void testFindTypeParameter() {
		Response response = target("expenseType").path("2")
				.path("typeParameter").path("2").register(JacksonFeature.class)
				.request().header("user", "demo").get();
		assertEquals(Status.OK.getStatusCode(), response.getStatus());
		TypeParameterSendto sendto = response
				.readEntity(TypeParameterSendto.class);
		assertEquals(2l, sendto.getId().longValue());
	}

	@Test
	public void test1FindTypeParameterWithNotFoundException() {
		Response response = target("expenseType").path("2")
				.path("typeParameter").path("4").register(JacksonFeature.class)
				.request().header("user", "demo").get();
	...
	public void testAssignTypeParameterToExpenseType() {
		Response response = target("expenseType")
				.property(ClientProperties.SUPPRESS_HTTP_COMPLIANCE_VALIDATION,
						true).path("2").path("typeParameter").path("1")
	...
	public void testRevokeTypeParameterToExpenseType() {
		Response response = target("expenseType")
				.property(ClientProperties.SUPPRESS_HTTP_COMPLIANCE_VALIDATION,
						true).path("2").path("typeParameter").path("2")
				.register(JacksonFeature.class).request()
